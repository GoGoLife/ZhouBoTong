...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self=[super initWithFrame:frame]) {
        ...
        [plusBtn setBackgroundImage:[UIImage imageNamed:@"fabu"] forState:UIControlStateNormal];
        [plusBtn setBackgroundImage:[UIImage imageNamed:@"fabu"] forState:UIControlStateHighlighted];
...
- (void)layoutSubviews
{
    ...
    //调整发布按钮的中线点Y值
    if (IS_IPHONE_X) {
        self.plusBtn.centerY = (self.height - 34) * 0.5 - 2*LBMagin;
    }else {
        self.plusBtn.centerY = self.height * 0.5 - 2*LBMagin;
    }
    
    self.plusBtn.size = CGSizeMake(self.plusBtn.currentBackgroundImage.size.width + 15, self.plusBtn.currentBackgroundImage.size.height + 15);
