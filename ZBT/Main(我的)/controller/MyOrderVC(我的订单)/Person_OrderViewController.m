...
@interface Person_OrderViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation Person_OrderViewController
...
- (void)getData {
    [self show];
    NSString *url = @"https://zbt.change-word.com/home/Member/personalOrder";
    ...
    [manager POST:url parameters:parme progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"personOrder +_+_+_+_+_+ ----%@", responseObject);
        [self hidden];
        [self.tableView.mj_header endRefreshing];
