//
//  Person_SellOrderViewController.m
//  ZBT
//
//  Created by 钟文斌 on 2018/9/30.
...
@interface Person_SellOrderViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation Person_SellOrderViewController
...
- (void)getData {
    [self show];
    NSString *url = @"https://zbt.change-word.com/home/member/personalSellOrder";
    ...
    [manager POST:url parameters:parme progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"person_SellOrder +_+_+_+_+_+ ----%@", responseObject);
        [self hidden];
        [self.tableView.mj_header endRefreshing];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dict = self.datatAry[indexPath.section];
    [cell.orderImg sd_setImageWithURL:[NSURL URLWithString:isNullClass(dict[@"photo"])] placeholderImage:[UIImage imageNamed:@"public"]];
    cell.nameTextF.text = [NSString stringWithFormat:@"*%@", isNullClass(dict[@"order_price"])];
    ...
    NSString *price = [isNullClass(dict[@"order_price"]) isEqualToString:@""] ? @"0" : dict[@"order_price"];
    CGFloat sum_price = 1 * [price floatValue];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    orderInfoVC.cellType = 7;
    orderInfoVC.orderInfoDic = self.datatAry[indexPath.section];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    textF.enabled = NO;
    textF.text = [@"    " stringByAppendingString:isNullClass(self.datatAry[section][@"username"])];
...
//    [pay setTitle:@"申请售后" forState:UIControlStateNormal];
...
- (void)payMoney:(UIButton *)sender {
    ...
    after.shopping_id = self.datatAry[sender.tag][@"id"];
    after.goodsArr = @[self.datatAry[sender.tag]];
    after.isShowButton = YES;
