...
@interface WaitGetViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation WaitGetViewController
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    [pay setTitle:@"确认收货" forState:UIControlStateNormal];
...
- (void)sendAction:(UIButton *)sender {
    NSString *url = @"https://zbt.change-word.com/index.php/home/order/changeOrder";
    AFHTTPSessionManager *manager = [[AFHTTPSessionManager alloc] init];
    manager.responseSerializer.acceptableContentTypes = [manager.responseSerializer.acceptableContentTypes setByAddingObject:@"text/html"];
    NSDictionary *parme = @{
                            @"order_id" : self.datatAry[sender.tag][@"order_id"],
                            @"state" : @(4)
                            };
    __weak typeof(self) weakSelf = self;
    [manager POST:url parameters:parme progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"waitPayOrder +_+_+_+_+_+ ----%@", responseObject);
        [ShowHUDView showHUDWithView:self.view AndTitle:@"已确认收货"];
        [weakSelf getData];
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
