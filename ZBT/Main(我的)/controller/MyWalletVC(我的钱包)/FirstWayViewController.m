...
- (void)setCellContent:(CustomTableViewCell *)cell AtIndexPaht:(NSIndexPath *)indexPath {
    ...
    CGFloat width = [self calculateRowWidth:@"支付宝账号" withFont:14];
    [cell.textF creatLeftView:FRAME(0, 0, width, InfoCellHeight) AndTitle:@[@"支付宝账号", @"体现金额"][indexPath.row] TextAligment:NSTextAlignmentLeft Font:SetFont(14) Color:[UIColor blackColor]];
    cell.textF.placeholder = @[@"请输入支付宝账号", @"请输入体现金额"][indexPath.row];
    [cell layoutIfNeeded];
}
StringWidth();
